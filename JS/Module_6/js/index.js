 ...
 * Попробуйте сделать это геттером чтобы можно было обращаться как obj.price и нам вернет сумму.*/
 calculatePrice(){
    ...
    const hamburgerToppingPrice = this._toppings.reduce((acc, value) => {
        return acc + Hamburger.TOPPINGS[value].price;
    }, 0);
    let hamburgerTotalPrice =
...
calculateCalories() {
    ...
    const hamburgerStuffingCalories =
        Hamburger.STUFFINGS[this.stuffing].calories;
    const hamburgerToppingCalories = this._toppings.reduce((acc, value) => {
        return acc + Hamburger.TOPPINGS[value].calories;
    }, 0);
    let hamburgerTotalCalories =
        hamburgerSizeCalories +
        hamburgerStuffingCalories +
        hamburgerToppingCalories;
    return hamburgerTotalCalories;
}
